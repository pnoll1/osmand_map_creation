...
def build(area_list):
    ...
    try:
        run(f'cd ../../osmand_map_creator;java -Djava.util.logging.config.file=logging.properties \
                -Xms64M -Xmx{XMX} -cp "./OsmAndMapCreator.jar:lib/OsmAnd-core.jar:./lib/*.jar" \
                net.osmand.util.IndexBatchCreator batch.xml', shell=True, capture_output=True, check=True,encoding='utf8')
